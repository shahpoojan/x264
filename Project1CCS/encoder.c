...
x264_t *x264_encoder_open   ( x264_param_t *param )
{
    ...
    if( h->param.rc.psz_stat_out )
        h->param.rc.psz_stat_out = (int) strdup( (char *)h->param.rc.psz_stat_out );
    if( h->param.rc.psz_stat_in )
        h->param.rc.psz_stat_in = (int) strdup( (char *)h->param.rc.psz_stat_in );
    if( h->param.rc.psz_rc_eq )
        h->param.rc.psz_rc_eq = (int) strdup( (char *)h->param.rc.psz_rc_eq );
#endif
    /* VUI */
